...
SOLVER_VERSION = "2026-08-07-jfm-hs-dsmc-restartable-v2"
...
class HardSphereQuarterCavity:
    ...
    def load_restart(self, restart_dir):
        ...
        self.cell_rng = cp.asarray(self._load_npy(
            restart_dir / "cell_rng.npy", np.uint64, counter_shape
        ))
        self.sigma_g_majorant = cp.asarray(self._load_npy(
            restart_dir / "sigma_g_majorant.npy", np.float32, counter_shape
        ))
        self.candidate_pairs = cp.asarray(self._load_npy(
            restart_dir / "candidate_pairs.npy", np.int64, counter_shape
        ))
        self.accepted_collisions = cp.asarray(self._load_npy(
            restart_dir / "accepted_collisions.npy", np.int64, counter_shape
        ))
        self.majorant_updates = cp.asarray(self._load_npy(
            restart_dir / "majorant_updates.npy", np.int64, counter_shape
        ))
    ...
    def save_restart(self, stem, completed_steps, sample_index):
        ...
        manifest = {
            ...
            "collision_state_is_complete": True,
